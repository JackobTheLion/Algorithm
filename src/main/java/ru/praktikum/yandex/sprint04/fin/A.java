 ...
 *
 * --ВРЕМЕННАЯ СЛОЖНОСТЬ--
 * При составлении индекса мы проходимся по каждому слову каждого документа, что составляет O(n * k), где n - количество
 * документов, а k - количество слов в документе.
 *
 * При обработке запроса в худшем случае, когда каждое слово встречается в каждом документе, мы получим сложность
 * O(m * t * n), где m - количество запросов, а t - количество слов в запросе.
 *
 * Т.о. общая сложность составит O(n * k + m * t * n) ~ O(m * n).
 *
